...
USHORT getOffsetOfFirstDataRunElement(byte* mftEntryBuffer, USHORT attributeHeaderOffset) {
	USHORT attributeNonResidentDataOffset = attributeHeaderOffset + g_BYTES_PER_ATTRIBUTE_HEADER; //arrive to the attribute non-resident data
	USHORT dataRunsOffset = 0;
...
void getMetadataOfDataRunElement(byte* mftEntryBuffer, USHORT offsetOfDataRunElement, uint8_t* pNumberOfClustersValueSize, uint8_t* pRelativeNumberOfStartingClusterValueSize) {
...
//note: "relative" relates to the previous data run element, or to zero if it is the first data run element  
void getInformationOfDataRunElement(byte* mftEntryBuffer, USHORT offsetOfDataRunElement, uint8_t numberOfClustersValueSize, uint64_t* pNumberOfClusters, uint8_t relativeNumberOfStartingClusterValueSize, int64_t* pRelativeNumberOfStartingCluster) {
	*pNumberOfClusters = 0;
...
DataRunsList* parseDataRuns(byte* mftEntryBuffer, USHORT attributeHeaderOffset) {
	if (isResident(mftEntryBuffer, attributeHeaderOffset)) { //make sure that the attribute is non-resident
		return 0;
	}

	USHORT offsetOfDataRunElement = getOffsetOfFirstDataRunElement(mftEntryBuffer, attributeHeaderOffset); //the offset is from the start of the MFT entry
